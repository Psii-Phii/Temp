...
class tmp117(object):
    ...
    def read_temp(self):
        try:
            ...
            pyb_uart.write_raw('Failed: '+ str(e))
            return None
        return None
...
def connect_tmp117(bus, add):
    ...
    while sensor == None:
        adds = bus.scan()
        if add in adds:
            pyb_uart.write_raw("Connecting to address " + str(add))
            return tmp117(bus, add)
        else:
            count += 1
            if count > 3:
                pyb_uart.write_raw("Failed to connect tmp117 at port " + str(add) + ". Returning None")
                return None
            pyb_uart.write_raw("Failed to connect tmp117 at port " + str(add) + ". Retrying (" + str(count) + "/3)")
            sensor = None
        pyb_uart.write_raw("Delaying 500ms to try again")
        delay(500)
