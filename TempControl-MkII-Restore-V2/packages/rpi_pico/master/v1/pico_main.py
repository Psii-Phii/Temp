...
tmps = []
for i in tmp_adds:
    t = pico_tmp117.connect_tmp117(i2c_bus, i)
    if not(t is None):
        pico_uart.write_raw('tmp117 connected at port ' + str(i))
        tmps.append(t)
    else:
        pico_uart.write_raw('tmp117 did not connect at port ' + str(i))
...
def measure_temperature(params):
    ...
    try:
        ...
        pico_uart.write_raw(params[0] + ' could not be interpreted as an integer. failed to read temperature.')
        return
